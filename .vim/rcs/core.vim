...
" 2000 Jun 1: for `Vim' 5.6
  ...
  " list:
"  global /'s$/ execute 'read ! echo ' . expand('<cword>') . TODO: UNCOMMENT
"    \ ' | ispell -l -d ' . g:IspellLang | delete            TODO: UNCOMMENT
...
autocmd FileType c,cabal,cpp,h,hpp,haskell,javascript,php,python,readme,text
...
if has("cscope")
  ...
  set cst
  set nocsverb
  " add any database in current directory, if not keep going
  " upwards in the directory tree until you find a cscope.out
  " file
...
:map <C-c><C-c> :exe ":cs find c " . expand("<cword>")<CR>
:map <C-c><C-g> :exe ":cs find g " . expand("<cword>")<CR>
:map <C-c><C-d> :exe ":cs find d " . expand("<cword>")<CR>
:map <C-c><C-e> :exe ":cs find e (^\|[^a-zA-Z_])" . expand("<cword>") . "([^a-zA-Z_]\|$)"<CR>
:map <C-c><C-a> :exe ":cs find e function " . expand("<cword>") . "([^a-zA-Z_]\|$)"<CR>
:map <C-c><C-b> :exe ":cs find s " . expand("<cword>")<CR>
:map <C-c><C-f> :cs find f
:map <C-c><C-t> :exe ":cs find t " . expand("<cword>")<CR>

function! s:reload_cscope_db()
  exe "set nocsverb"
  exe "cs reset"
  exe "set csverb"
endfunction
...
let g:csquick_on = 0
function! s:toggle_cscope_quickfix()
  if g:csquick_on == 0
...
:map <C-c><C-r> :call <SID>reload_cscope_db()<CR>
:map <C-c><C-x> :call <SID>toggle_cscope_quickfix()<CR>

" My stuff
:let mapleader = ","

" Commenting stuff
" autocmd FileType php map ,/ :s/^/\/\//<CR>:s/sldfkjsdlfkjsdlkfjsdkfljsdlkf<CR>
...
map TV :call Damn("tbgs")<CR>
"****************************************************

"******************* FROM PAULSHEN ******************
noremap <CR> <CR>:cclose<CR>

"This is to fix the comment stuff for php
if has("autocmd") 
